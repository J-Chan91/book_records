...
export default function RecordsSection({ list }: Props) {
  ...
  return (
    <Container
      isEmptyRecords={!records?.length}
      onUpdateRecords={updateRecords}
    >
      <div
        className={cn(
          "h-full flex flex-col items-center gap-2 overflow-auto",
      ...
      >
        {!records ? (
          ...
          <div className="w-full">
            <div className="w-full justify-center grid grid-cols-1 xl:grid-cols-3 gap-2">
              {records.map((item) => (
                <Link
                  key={item.id}
                  className=" bg-white w-full md:flex rounded border border-gray-200 px-2 py-1 transition cursor-pointer hover:border-gray-400 h-[320px]"
                  href={`/${item.id}`}
                >
                  <Image
                    src={item.image}
